...
class CommunicationChannel:
    ...
    def __init__(
        ...
        if self.comm_type == "graph":
            ...
            if agent_ids is not None and topology.shape[0] != len(agent_ids):
                raise ValueError(f"Topology shape {topology.shape} doesn't match number of agents {len(agent_ids)}")
    ...
    def send(
        ...
        if message.shape[-1] != self.message_dim:
            raise ValueError(f"Message dimension {message.shape[-1]} doesn't match expected {self.message_dim}")
...
class MultiAgentCommunicationWrapper:
    ...
    def __init__(
        self,
        policies: Dict[str, CommunicatingPolicy],
        comm_channel: CommunicationChannel
    ):
