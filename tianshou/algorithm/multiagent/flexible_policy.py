...
class FlexibleMultiAgentPolicyManager(MultiAgentPolicy):
    ...
    def __init__(
        self,
        policies: Union[Policy, List[Policy], Dict[str, Policy]],
    ...
    def _validate_config(self, policies):
        ...
        if self.mode == "independent":
            if isinstance(policies, Policy):
                raise ValueError("Independent mode requires list or dict of policies, got single policy")
    ...
    def _build_policy_map(self, policies, agents) -> Dict[str, Policy]:
        ...
        if self.mode == "shared":
            ...
            return {agent: shared_policy for agent in agents}
            ...
            if isinstance(policies, list):
                if len(policies) != len(agents):
                    raise ValueError(f"Number of policies ({len(policies)}) must match number of agents ({len(agents)})")
    ...
    @override
    def forward(self, batch: Batch, state: Union[dict, Batch, None] = None, **kwargs: Any) -> Batch:
    ...
    def _forward_shared(self, batch: Batch, state: Union[dict, Batch, None] = None, **kwargs: Any) -> Batch:
