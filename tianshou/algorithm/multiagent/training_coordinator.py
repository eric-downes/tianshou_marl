    ...
    def __init__(
        ...
        **kwargs
        ...
        if training_mode not in self.VALID_MODES:
            raise ValueError(f"Invalid training mode: {training_mode}. Must be one of {self.VALID_MODES}")
...
class SimultaneousTrainer(MATrainer):
    ...
    def __init__(
        self,
        policy_manager: FlexibleMultiAgentPolicyManager,
        agent_train_freq: Optional[Dict[str, int]] = None,
        replay_buffers: Optional[Dict[str, ReplayBuffer]] = None,
        **kwargs
    ):
...
class SelfPlayTrainer(MATrainer):
    ...
    def _sample_opponent(self) -> Optional[Policy]:
    ...
    def update_win_rate(self, opponent_id: int, won: bool):
        ...
        self.opponent_win_rates[opponent_id] = (
            alpha * (1.0 if won else 0.0) + 
            (1 - alpha) * self.opponent_win_rates[opponent_id]
        )
...
class LeaguePlayTrainer(MATrainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.agent_performance = {agent: 0.5 for agent in self.league}
        self.elo_ratings = {agent: 1000 for agent in self.league}
        self.game_count = 0
    ...
    def update_match_result(self, winner: str, loser: str):
        ...
        # Update performance
        alpha = 0.1
        self.agent_performance[winner] = (
            alpha * 1.0 + (1 - alpha) * self.agent_performance[winner]
        )
        self.agent_performance[loser] = (
            alpha * 0.0 + (1 - alpha) * self.agent_performance[loser]
        )
