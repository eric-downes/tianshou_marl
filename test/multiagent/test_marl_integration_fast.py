...
class FastMockEnv:
    ...
    def __init__(self, n_agents=2, parallel=True):
        ...
        self.observation_spaces = {
            agent: spaces.Box(low=-1, high=1, shape=(4,)) for agent in self.agents
        }
...
class TestFastMARL:
    ...
    def test_dict_observation_processing(self):
        ...
        obs_space = spaces.Dict(
            {"position": spaces.Box(low=-1, high=1, shape=(2,)), "state": spaces.Discrete(3)}
        )
        ...
        # Single observation
        obs = {"position": np.array([0.5, -0.5]), "state": np.array(1)}
        batch = Batch(obs=obs)
    ...
    def test_training_coordinator_basic(self):
        ...
        policies = {"agent_0": FastMockPolicy(), "agent_1": FastMockPolicy()}
    ...
    def test_global_state_construction(self):
        ...
        constructor = GlobalStateConstructor(mode="concatenate", obs_dim=4, n_agents=2)

        observations = {"agent_0": torch.randn(10, 4), "agent_1": torch.randn(10, 4)}
    ...
    def test_integration_dict_obs_with_ctde(self):
        ...
        obs_space = spaces.Dict(
            {"local": spaces.Box(low=-1, high=1, shape=(3,)), "id": spaces.Discrete(2)}
        )
        ...
        # Simple CTDE setup
        actor = nn.Sequential(nn.Linear(preprocessor.output_dim, 16), nn.ReLU(), nn.Linear(16, 2))
        ...
        obs_dict = {"local": torch.randn(1, 3), "id": torch.tensor([0])}
