...
class TestGlobalStateConstructor:
    ...
    def test_graph_based_constructor(self):
        ...
        constructor = GlobalStateConstructor(
            mode="graph", obs_dim=4, n_agents=3, adjacency_matrix=adjacency
        )
    ...
    def test_custom_constructor(self):
        ...
        constructor = GlobalStateConstructor(mode="custom", custom_fn=custom_builder)
...
class TestQMIX:
    ...
    def test_qmix_initialization(self):
        ...
        mixer = QMIXMixer(n_agents=n_agents, state_dim=12, mixing_embed_dim=32)
    ...
    def test_qmix_mixing_network(self):
        ...
        mixer = QMIXMixer(n_agents=n_agents, state_dim=state_dim, mixing_embed_dim=32)
...
class TestMADDPG:
    ...
    def test_maddpg_centralized_critic(self):
        ...
        batch = create_mock_batch(
            n_agents=n_agents, with_global_state=True, action_space=action_space
        )
        ...
        for episode in range(10):
            ...
            for step in range(10):
                ...
                for i, agent in enumerate(env.agents):
                    batch[agent] = Batch(
                        obs=torch.tensor(obs[agent], dtype=torch.float32).unsqueeze(0)
                    )

                actions = policy.forward(batch)

                # Environment step
                action_dict = {agent: actions[agent].act.item() for agent in env.agents}
                ...
                experience = create_mock_batch(
                    n_agents=env.n_agents, batch_size=1, with_global_state=True
                )
